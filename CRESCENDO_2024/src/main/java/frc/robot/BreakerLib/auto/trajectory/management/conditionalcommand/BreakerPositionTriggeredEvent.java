...
public class BreakerPositionTriggeredEvent implements BreakerConditionalEvent{
    ...
    public boolean checkCondition(double currentTimeSeconds, Pose2d currentPose) {
        boolean satX = BreakerMath.epsilonEquals(triggerPose.getX(), currentPose.getX(), Math.abs(tolerences.getX()));
        boolean satY = BreakerMath.epsilonEquals(triggerPose.getY(), currentPose.getY(), Math.abs(tolerences.getY()));
        boolean satRot = BreakerMath.epsilonEquals(triggerPose.getRotation().getDegrees(), currentPose.getRotation().getDegrees(), Math.abs(tolerences.getRotation().getDegrees()));
